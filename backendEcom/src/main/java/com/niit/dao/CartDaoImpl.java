...
public List<Cart> getByCustomer(Customer c) {
	...
	Session session = sessionFactory.openSession();
	String hql = "from Cart where customer.custId="+c.getCustId();
...
public boolean deleteByCustomer(Customer c) {
	// TODO Auto-generated method stub
	Session session = sessionFactory.openSession();
	String hql = "delete from Cart where customer.custId="+c.getCustId();
	Query query = sessionFactory.openSession().createQuery(hql);
	query.executeUpdate();
...
public Double getTotalAmount(Customer c) {
	String hql = "select sum(product.price) from Cart where customer.custId="+c.getCustId();
	  Session session = sessionFactory.openSession();
